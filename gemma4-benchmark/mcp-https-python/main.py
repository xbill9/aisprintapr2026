...
async def manage_vllm_docker(resource_id: str = "vllm-gemma4-q4", action: str = "start") -> str:
    ...
    docker_run_cmd = (
        ...
        f"--max-model-len 16384 --dtype bfloat16 --kv-cache-dtype fp8 --gpu-memory-utilization 0.90 --block-size 32 --disable_chunked_mm_input --tensor-parallel-size {TENSOR_PARALLEL_SIZE} "
...
async def estimate_deployment_cost(
    hours: float = 1.0, tpu_type: str = "v6e", topology: str = "1x4", is_flex: bool = True
) -> str:
...
async def run_context_benchmark(
    ...
    This tool evaluates the model's latency as the input prompt size increases.
    It returns a formatted table of results suitable for analysis.
    """
    url = await discover_vllm_url()
    if not url:
        return "❌ No ACTIVE Queued Resource with a reachable vLLM service found."

    # Add parent dir to path to find context_benchmark
    import sys

    sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
    from context_benchmark import ContextBenchmark
    concurrencies = [int(c.strip()) for c in concurrency.split(",")]

    # Generate lengths: linear steps
    step_size = max(1, max_context // steps)
    lengths = list(range(step_size, max_context + 1, step_size))
    if 128 not in lengths and 128 < max_context:
        lengths.insert(0, 128)
    if lengths and lengths[-1] != max_context:
        lengths.append(max_context)

    benchmark = ContextBenchmark(url, model)

    results = []
    async with httpx.AsyncClient() as client:
        for c in concurrencies:
            for length in lengths:
                res = await benchmark.run_single_test(client, length, c)
                if res["success"]:
                    results.append({
    ...
    if not results:
        return "❌ Benchmark failed: No results gathered."

    df = pd.DataFrame(results)
    summary_table = df[["prompt_tokens", "concurrency", "avg_ttft", "prefill_tps"]].to_markdown(index=False)

    return f"### 📊 Multi-Dimensional Context Benchmark Results\n- **Model:** {model}\n- **Endpoint:** {url}\n\n{summary_table}\n\n*Note: avg_ttft is in seconds. prefill_tps is batch throughput in tokens/sec.*"


@mcp.tool()
async def get_vllm_docker_logs(resource_id: str = "vllm-gemma4-q4", tail: Optional[int] = None) -> str:
...
async def get_tpu_system_logs(
    resource_id: str = "vllm-gemma4-q4", service: str = "docker", tail: Optional[int] = None
) -> str:
